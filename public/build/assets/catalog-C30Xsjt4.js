...
Apakah produk ini masih tersedia? Terima kasih.`,window.open(`https://wa.me/${s}?text=${encodeURIComponent(e)}`,"_blank")})}function B(){$(window).scroll(function(){$(this).scrollTop()>100?$("#btn-scroll-top").fadeIn():$("#btn-scroll-top").fadeOut()}),$("#btn-scroll-top").click(function(){return $("html, body").animate({scrollTop:0},500),!1})}function U(){$("#loading").removeClass("d-none")}function W(){$("#loading").addClass("d-none")}function D(t=[]){if(!Array.isArray(t))return[];const s={thumbnail:3,motif:2,product:1};return t.slice().sort((a,e)=>(s[e.type]||0)-(s[a.type]||0)).map(a=>"/storage/"+a.path)}function Y(t,s){$("#btn-download").removeClass("d-none");let a="";t.forEach(e=>{var p,d,f,v,P,b,y,T,k,S,J,C,L,m;const i=D(((d=(p=e==null?void 0:e.product_versions)==null?void 0:p[0])==null?void 0:d.images)??[]),n=i.length?i[0]:"https://via.placeholder.com/300x200?text=No+Image",h=JSON.stringify(i).replace(/"/g,"&quot;"),c=JSON.stringify(e.packages).replace(/"/g,"&quot;"),l=((f=e.category)==null?void 0:f.name)??"Tanpa Kategori";((v=e.category)==null?void 0:v.display_style)==="square"||s===null?a+=`
                    ...
                    `;const r=((b=e.category)==null?void 0:b.display_style)==="rectangle",u=JSON.stringify(e.specifications).replace(/"/g,"&quot;");a+=`
                    data-id="${e.id}"
                    data-code="${e.code}"
                    data-name="${e.name}"
                    data-category="${l}"
                    ...
                    data-images="${h}"
                    data-image="${n}"
                    data-type="${((S=(k=e.category)==null?void 0:k.types)==null?void 0:S.name)??""}"
                    ...
                    >
                       <img 
                            ...
                            style="
                                ...
                                width: 100%; 
                                object-fit: cover; 
                                object-position: ${((m=(L=e.category)==null?void 0:L.jenis)==null?void 0:m.name)==="PVC Board"?"bottom center":"center center"};
        ...
        `)}),$("#mockup").removeClass("d-none");return}i.length>0?(i.length===1?($("#mockup .carousel-control-next").addClass("d-none"),$("#mockup .carousel-control-prev").addClass("d-none"),g.addClass("d-none")):($("#mockup .carousel-control-next").removeClass("d-none"),$("#mockup .carousel-control-prev").removeClass("d-none"),g.removeClass("d-none")),i.forEach((l,r)=>{n.append(`
                <div class="carousel-item ${r===0?"active":""}">
            ...
            `)}),$("#mockup").removeClass("d-none")):$("#mockup").addClass("d-none")}let _=!1;function F(t){$(document).off("click",".category-filter"),$(document).on("click",".category-filter",async function(s){if(s.preventDefault(),_)return;_=!0;const a=$(this).data("id"),e=$(this).data("type");try{R({selectedJenis:t,category:a,type:e}),$("#filterModal").modal("hide"),V(),await w(!1)}catch(i){console.error("Gagal memuat data:",i)}finally{_=!1}})}function x(t,s="#modalVariants"){$(s).empty(),$("#modalPaket").empty(),$("#modalKepadatan").empty(),Array.isArray(t)&&t.length>0?t.forEach(a=>{const e=a.specification_values||[],i=a.name.toLowerCase();if(i==="warna")$("#paket").text("Warna"),e.length>0?e.forEach(n=>{a.pivot&&n.id===a.pivot.specification_value_id&&$("#modalPaket").append(`
                            ...
                            `)}):$("#modalPaket").append('<span class="text-muted">Tidak ada data</span>');else if(i==="density")e.length>0?e.forEach(n=>{a.pivot&&n.id===a.pivot.specification_value_id&&(e.length>1?$("#modalKepadatan").append(`
                                ...
                                `):$("#modalKepadatan").append(n.name))}):$("#modalKepadatan").append('<span class="text-muted">Tidak ada data</span>');else{const n=e.find(l=>a.pivot&&l.id===a.pivot.specification_value_id),g=n?n.name:"-";let h=n?n.unit:"";h?h=" "+h:["panjang","tinggi","lebar"].includes(i)?h=" cm":i==="ketebalan"&&(h=" mm");const c=`
                    <div class="row mb-1 variant-row">
                        <div class="col-4 col-sm-3 font-weight-bold">${a.name}</div>
                        <div class="col-auto">:</div>
                        <div class="col">${g}${h}</div>
                    </div>
                `;$(s).append(c)}}):($(s).append('<div class="text-muted">Tidak ada varian</div>'),$(".paket").hide())}function G(){$("#version-select").on("change",function(){const t=$(this).val()||null;o.version=t,V(),w()})}function q({selectedJenis:t=null,hasCategory:s=!1,isRenderTypes:a=!1}){const e=$("#filter-container"),i=$("#category-container"),n=$("#catalog-col"),g=$(window).width()<768;if(e.removeClass("d-none"),i.removeClass("d-none"),n.removeClass("col-md-12 col-md-9"),a){e.addClass("d-none"),i.addClass("d-none"),n.addClass("col-md-12");return}if(t==1){e.addClass("d-none"),i.addClass("d-none"),n.addClass("col-md-12");return}if(!s){e.addClass("d-none"),i.addClass("d-none"),n.addClass("col-md-12");return}g?(e.addClass("d-none"),i.removeClass("d-none")):(e.removeClass("d-none"),i.removeClass("d-none")),n.addClass("col-md-9")}$(document).ready(function(){const t=new URLSearchParams(window.location.search),s=t.get("jenis");t.get("version");const a=t.get("category");let e,i,n=!1;R({selectedJenis:s,category:a}),w(),q({selectedJenis:s,hasCategory:a,isRenderTypes:!1}),$(window).on("resize",()=>q({selectedJenis:s,hasCategory:a,isRenderTypes:!1})),$("#search-input").on("input",function(){clearTimeout(e),e=setTimeout(()=>{V(),w()},500)}),$(window).on("scroll",function(){clearTimeout(i),i=setTimeout(async()=>{if(n||H())return;const c=$(window).scrollTop(),l=$(window).height(),r=$(document).height();if(c+l>=r-150){n=!0;try{M(!1),await w()}finally{n=!1}}},200)});function g(c,l=[],r=""){$("#carousel-product-image").empty(),$("#thumbnailGallery").empty();let u=c||[];(r==null?void 0:r.toLowerCase())==="uv board"&&l.length&&(u=[...c,...l]),u.forEach((d,f)=>{const v=f===0?"active":"";$("#carousel-product-image").append(`
            ...
            `)}),u.length<=1?$("#carouselProduct .carousel-control-next, #carouselProduct .carousel-control-prev").addClass("d-none"):$("#carouselProduct .carousel-control-next, #carouselProduct .carousel-control-prev").removeClass("d-none"),$("#carouselProduct").carousel({interval:3e3,pause:!1});let p;$("#thumbnailGallery").off("click").on("click",".thumbnail-image",function(){const d=$(this).data("index");$("#carouselProduct").carousel(d),$(".thumbnail-image").removeClass("active-thumbnail"),$(this).addClass("active-thumbnail"),setTimeout(()=>$(this).removeClass("active-thumbnail"),500),$("#carouselProduct").carousel("pause"),clearTimeout(p),p=setTimeout(()=>{$("#carouselProduct").carousel("cycle")},5e3)})}$(document).off("click",".product-card").on("click",".product-card",function(){const c=$(this).data("id"),l=$(this).data("code"),r=$(this).data("name"),u=$(this).data("category"),p=$(this).data("type"),d=$(this).data("jenis"),f=parseInt($(this).data("length"),10),v=parseFloat($(this).data("width")).toFixed(1),P=parseFloat($(this).data("density")).toFixed(1),b=$(this).data("url"),y=JSON.parse($(this).attr("data-specifications")||"[]"),T=$(this).attr("data-images"),k=JSON.parse($(this).attr("data-paket")||"[]");let S=$(this).data("height"),J=$(this).data("type-image"),C=[],L=[];if(d!=="card-types"&&h(c),T)try{C=JSON.parse(T.replace(/&quot;/g,'"'))}catch(m){C=[],console.error(m)}if(Array.isArray(k)&&(L=k.sort((m,j)=>m.order-j.order).map(m=>`/storage/${m.image}`)),$("#notes").show(),$("#modalPaket").empty(),d==="card-types"){V(),R({selectedJenis:s,type:p}),M(!1),w();return}d.toLowerCase()==="uv board"&&(x(y),$("#modalVideo, .lebar").hide(),$("#modalCategory").text(u),k.length>0?k.sort((m,j)=>m.order-j.order).map(m=>{$("#modalPaket").append(`
